...
//	printData(*d_D,length);
	...
	cutilSafeCall(cudaFreeHost(sptemp)); 	
...
void findComponent(Vertex *v,unsigned int * D, unsigned int length){
	...
	cutilSafeCall( cudaMalloc( (void**) &d_v, length* sizeof(Vertex)) );	
	cutilSafeCall( cudaMalloc( (void**) &d_curD, length* sizeof(int) ));	
	
	cutilSafeCall( cudaMemcpy(d_v, v, length*(sizeof(Vertex)), cudaMemcpyHostToDevice) );
	findComponentDevice(d_v,&d_curD,length);

	cutilSafeCall( cudaMemcpy(D, d_curD, length*(sizeof(int)), cudaMemcpyDeviceToHost) );
	cutilSafeCall( cudaFree(d_v) );	
	cutilSafeCall( cudaFree(d_curD) );	
}
