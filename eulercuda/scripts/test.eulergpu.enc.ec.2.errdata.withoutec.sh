...
outputDir=./$dirPrefix
eulerCudaApp=/home/smahmood/test/phase2/transform/eulercuda/bin/linux/release/eulercuda
inputDir=$1
origDir=$2
seqK=`seq 10 2 32`
seqT=`seq 20 2 32`
...
do
echo $r
	for f in $inputDir/*$r*bp.fas*					# iterate over each input
	do 
		echo $f
		...
		do
			...
			do
				...
				T1=`date +%s.%N`
			#	$eulerCudaApp -i $f -o $outputFile  -r $r -e -t $kmer -m 1  1>>$statFile  2>> $logFile		# Run CUDA EC
				$eulerCudaApp -i $f -o $outputFile  -r $r -a -l $kmer -b $block  1>>$statFile  2>> $logFile		# Run CUDA EC
				T2=`date +%s.%N`
