...
CXX        := g++
CC         := gcc
LINK       := g++ -fPIC -static-libgcc #-static
...
NVCCFLAGS +=  --ptxas-options="-v" --compiler-options -static# --compiler-bindir /opt/gcc34/ 
CXXFLAGS  := $(CXXWARN_FLAGS)  -rdynamic #-static
CFLAGS    := $(CWARN_FLAGS) -rdynamic #-static 


# Common flags
COMMONFLAGS += $(INCLUDES) -DUNIX   #-malign-double

# Debug/release configuration
ifeq ($(dbg),1)
	COMMONFLAGS += -g
	NVCCFLAGS   += -D_DEBUG  #--device-debug 3 --compiler-options=' -g3 -ggdb '
	CXXFLAGS    += -D_DEBUG -g3 -ggdb
	CFLAGS      += -D_DEBUG
	BINSUBDIR   := debug
	LIBSUFFIX   := #D
else 
	...
	BINSUBDIR   := release
	LIBSUFFIX   :=
	NVCCFLAGS   += --compiler-options -fno-strict-aliasing  -DEULER_NDEBUG
	CXXFLAGS    += -fno-strict-aliasing -DEULER_NDEBUG
	CFLAGS      += -fno-strict-aliasing -DEULER_NDEBUG
endif
...
# static linking, we will statically link against CUDA and CUDART
  ifeq ($(USEDRVAPI),1)
     LIB += -lcuda -lcudart  ${OPENGLLIB} $(PARAMGLLIB) $(RENDERCHECKGLLIB) $(CUDPPLIB) ${LIB} 
  else
...
##LIB += -L$(BOOST_LIB)
##LIB += -Wl,-Bstatic -lboost_program_options 
...
#LIB += -L$(BOOST_LIB)
LIB += $(GPUEULER_LIB)   -lboost_program_options-mt -Wl,-Bdynamic 
