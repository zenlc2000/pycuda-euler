...
void printSuccessorGraph(Vertex * d_v , unsigned int length){
	...
	cutilSafeCall(cudaMemcpy(h_v,d_v,length*sizeof(Vertex),cudaMemcpyDeviceToHost));
	printf("$graph G {\n");
...
void printCircuitGraph(CircuitEdge * d_ce , unsigned int length){
	...
	cutilSafeCall(cudaMemcpy(h_ce,d_ce,length*sizeof(CircuitEdge),cudaMemcpyDeviceToHost));
	printf("$graph G {\n");
	for (unsigned int i =0; i< length; i++){	
...
void validateSuccessors(EulerEdge * d_ee, unsigned int ecount) {
	...
	cutilSafeCall(cudaMemcpy(h_ee,d_ee,ecount * sizeof(EulerEdge),cudaMemcpyDeviceToHost));
...
void executeSwipeHostLaunch(EulerVertex * d_ev, unsigned int * d_e, unsigned int vcount, EulerEdge * d_ee, unsigned int * d_mark , unsigned int ecount){
		...
		cutilSafeCall( cudaMemcpy(h_ev,d_ev, vcount*sizeof(EulerVertex), cudaMemcpyDeviceToHost));
		cutilSafeCall( cudaMemcpy(h_e,d_e, vcount*sizeof(unsigned int), cudaMemcpyDeviceToHost));
		cutilSafeCall( cudaMemcpy(h_ee,d_ee, ecount*sizeof(EulerEdge), cudaMemcpyDeviceToHost));
		cutilSafeCall( cudaMemcpy(h_mark,d_mark, ecount*sizeof(unsigned int), cudaMemcpyDeviceToHost));
...
//	printData(d_cv,circuitVertexSize);
	...
	//	cutilSafeCall( cudaMemcpy(h_cedgeCount, d_cedgeCount, circuitVertexSize*sizeof(unsigned int), cudaMemcpyDeviceToHost));
		getOptimalLaunchConfiguration(vcount,&grid,&block);
	...
	//	cutilSafeCall( cudaMemcpy(d_cedgeCount,h_cedgeCount, circuitVertexSize*sizeof(unsigned int), cudaMemcpyHostToDevice));
	//	free(h_cedgeCount);
...
/*		getOptimalLaunchConfigCustomized(circuitVertexSize,&grid,&block,1);
		...
		cutilSafeCall( cudaMemcpy(h_cg_edge,*d_cg_edge, circuitGraphEdgeCount*sizeof(CircuitEdge), cudaMemcpyDeviceToHost));
		std::sort(h_cg_edge,h_cg_edge+circuitGraphEdgeCount,edgeComp);
		cutilSafeCall( cudaMemcpy(*d_cg_edge,h_cg_edge, circuitGraphEdgeCount*sizeof(CircuitEdge), cudaMemcpyHostToDevice));		
		free(h_cg_edge);	
